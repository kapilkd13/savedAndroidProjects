 ...
 */
public class DownloadFAQIndexFile
        extends AsyncTask<ArrayList<String>, Integer, String> {
    private Context context;
    private Fragment fragment;
    private  boolean StatusOK=false;
    public DownloadFAQIndexFile(Context context,Fragment f)
    ...
    private String uploadFile(ArrayList<String> param) {
        ...
        HttpPost httppost = new HttpPost(Constants.FAQ_INDEX_FILE_URL);
...
//here set unique id to response from server
    ...
    protected void onPostExecute(String result) {
        Log.e("on postExecute", "Response from faq index server: " + result);

        // showing the server response in an alert dialog
        try {
            if(StatusOK) {
//here copy response to file
                if (!(new DownloadedFileAddress().checkFilePresence("FAQIndex.json", Constants.FAQ, context))) {//create new file
                    File file = new File(new DownloadedFileAddress().getNewFileAddress("FAQIndex.json", Constants.FAQ, context));
                    if (!file.exists()) {
                ...
                File file = new File(new DownloadedFileAddress().getDownloadedFileAddress("FAQIndex.json", Constants.FAQ, context));
                if (file.exists()) {
                    FileWriter writer = new FileWriter(file);
                    writer.write(result);
                    writer.close();
                    ((FAQExpandableListFragment)fragment).isLatestIndexFile=true;
                } else {
...
//upload image and video here
        ...
        {((FAQExpandableListFragment)fragment).afterphpCall();}
